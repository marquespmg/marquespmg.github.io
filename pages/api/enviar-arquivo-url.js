// pages/api/enviar-arquivo-url.js
import { supabase } from '../../lib/supabaseClient';

export default async function handler(req, res) {
  if (req.method !== 'POST') return res.status(405).end();

  const { telefone, urlArquivo, nomeArquivo, legenda } = req.body;

  if (!telefone || !urlArquivo) {
    return res.status(400).json({ erro: 'Telefone e URL são obrigatórios' });
  }

  const TOKEN = process.env.WHATSAPP_TOKEN;
  const PHONE_ID = process.env.WHATSAPP_PHONE_ID;

  try {
    const resp = await fetch(
      `https://graph.facebook.com/v21.0/${PHONE_ID}/messages`,
      {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${TOKEN}`,
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          messaging_product: 'whatsapp',
          to: telefone,
          type: 'document',
          document: {
            link: urlArquivo,
            filename: nomeArquivo,
            caption: legenda || ''
          }
        })
      }
    );

    const data = await resp.json();

    // Salva no Supabase
    let { data: contato } = await supabase
      .from('contatos')
      .select('id')
      .eq('telefone', telefone)
      .single();

    await supabase.from('mensagens').insert({
      contato_id: contato?.id || null,
      telefone,
      direcao: 'enviada',
      tipo: 'documento',
      conteudo: legenda || `[Arquivo: ${nomeArquivo}]`,
      message_id: data.messages?.[0]?.id || null,
      status: resp.ok ? 'sent' : 'failed',
      erro: resp.ok ? null : (data.error?.message || null)
    });

    await supabase.from('conversas').upsert({
      contato_id: contato?.id || null,
      telefone,
      ultima_mensagem: `📎 ${nomeArquivo}`,
      ultima_mensagem_em: new Date().toISOString(),
      ultima_mensagem_direcao: 'enviada'
    }, { onConflict: 'telefone' });

    if (!resp.ok) {
      return res.status(resp.status).json({ erro: data });
    }

    return res.status(200).json({ ok: true, messageId: data.messages?.[0]?.id });
  } catch (err) {
    return res.status(500).json({ erro: err.message });
  }
}